...
class LatestPostsBlock extends Component {
	...
	render() {
		...
		if ( ! hasPosts ) {
			return (
				<Fragment>
					...
					<Placeholder
						icon="admin-post"
						label={ __( 'Cortex Blocks Post and Page Grid', 'covertnine-blocks' ) }
					>
						{ ! Array.isArray( latestPosts ) ?
							<Spinner /> :
							__( 'No posts found.', 'covertnine-blocks' )
						}
					</Placeholder>
				</Fragment>
			);
		...
		const layoutControls = [
			{
				icon: 'grid-view',
				title: __( 'Grid View', 'covertnine-blocks' ),
				onClick: () => setAttributes( { postLayout: 'grid' } ),
				isActive: attributes.postLayout === 'grid',
			},
			{
				icon: 'list-view',
				title: __( 'List View', 'covertnine-blocks' ),
							...
							>
								{
									attributes.displayPostImage && post.featured_media ? (
										<PostGridImage
											{ ...this.props }
											imgAlt={ decodeEntities( post.title.rendered.trim() ) || __( '(Untitled)', 'covertnine-blocks' ) }
								...
								<div className="ab-block-post-grid-text">
									<header className="ab-block-post-grid-header">
										{ attributes.displayPostTitle &&
											<PostTag className="ab-block-post-grid-title"><a href={ post.link } target="_blank" rel="bookmark">{ decodeEntities( post.title.rendered.trim() ) || __( '(Untitled)', 'covertnine-blocks' ) }</a></PostTag>
										}
											...
											<div className="ab-block-post-grid-byline">
												...
												{ attributes.displayPostDate && post.date_gmt &&
													<time dateTime={ moment( post.date_gmt ).utc().format() } className={ 'ab-block-post-grid-date' }>
														{ moment( post.date_gmt ).local().format( 'MMMM DD, Y', 'covertnine-blocks' ) }
													</time>
												}
											</div>
										}
...
export default compose( [
	withSelect( ( select, props ) => {
		...
		const { getEntityRecords } = select( 'core', 'covertnine-blocks' );
